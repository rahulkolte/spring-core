...
public class ReadMultipleBeans {

	private static Map<String, Object> hmapBeans = new HashMap<String, Object>();
  static int totalPrice=0;
	public static void main(String args[]) throws Exception {
		NodeList nodeList = readXmlFile("C:\\Users\\RK61202\\Documents\\workspace-spring-tool-suite-4-4.10.0.RELEASE\\Day2\\src\\main\\java\\multiple-beans.xml");
		readBeans(nodeList);
				...
				for (int j = 0; j < propertyNodeList.getLength(); j++) {
					...
					if (propertyNode.getNodeType() == Node.ELEMENT_NODE) {
						...
						if (propertyElement.hasAttribute("value")) {
							...
							if(j==1)
							{
								totalPrice=totalPrice+Integer.valueOf(propertyValue);
							}
						...
						/*
						 * for (Method method : beanMethods) { if (method.getName().equals(methodName))
						 * { Class paramTypes[] = method.getParameterTypes(); Class paramType =
						 * paramTypes[0]; if (paramType.getName().equals("java.lang.Double")) {
						 * paramValues[0] = Double.valueOf((String) paramValues[0]); } else if
						 * (paramType.getName().equals("java.lang.Integer")) { paramValues[0] =
						 * Integer.valueOf((String) paramValues[0]); } method.invoke(beanObj,
						 * paramValues); break; } }
						 */
					}
				}
